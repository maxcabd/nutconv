...
from dds import NutTexture_to_DDS, DDS_to_NutTexture, write_dds,write_png, texture_565, texture_5551, texture_4444
from tkinter import Menu, ttk, Grid, filedialog as fd
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.upper_frame1 = ttk.Frame(master=self.window, width=275, height=50,)
        ...
        self.upper_frame2 = ttk.Frame(master=self.window, width=275, height=50,)
        ...
        self.export_texture.menu.add_command(label="Export as NUT", command= self.export_nut)
        self.export_texture.menu.add_command(label="Export as DDS", command= self.export_dds)
        self.export_texture.menu.add_command(label="Export as PNG", command= self.export_png)
        ...
        self.lists_frame = ttk.Frame(master=self.window, width=550, height=350) #corner_radius=10,)
        ...
        #treeview
        self.textures_list = ttk.Treeview(master=self.lists_frame, height=14, columns=("Count"), selectmode="extended")
        #self.textures_list.grid(row=0, column=1, sticky='nsew', padx=5, pady=5)
        self.textures_list.pack(side=tkinter.LEFT, fill=tkinter.BOTH, expand=True)
        ...
        self.texture_preview_frame = ttk.LabelFrame(master=self.window, width=350, height=350, text="Texture Preview")
        ...
        #Texture info frame
        self.texture_info_frame = ttk.LabelFrame(master=self.window, width=350, height=100, text="Texture Info")
        ...
        #-----------------------------------------------------------------------------------------------------------------------
        #lower buttons
        self.lower_buttons_frame1 = ttk.Frame(master=self.window, width=275, height=50,)
        self.lower_buttons_frame1.grid(row=2, column=0, sticky='nsew', padx=5, pady=5)
        ...
        self.lower_buttons_frame2 = ttk.Frame(master=self.window, width=275, height=50,)
        self.lower_buttons_frame2.grid(row=2, column=1, sticky='nsew', padx=5, pady=5)
        ...
        self.lower_frame.grid(row=3, column=0, sticky='nsew', padx=5, pady=5, columnspan=2)
        ...
        self.nut_menu.add_command(label="Copy NUT Texture", command=self.copy_nut_texture)
        self.nut_menu.add_command(label="Paste NUT Texture", command=self.paste_nut_texture)
        self.nut_menu.add_command(label="Add Textures", command=self.add_texture)

        self.texture_menu = Menu(self.window, tearoff=0)
        self.texture_menu.add_command(label="Replace Texture", command=self.replace_texture)
        self.texture_menu.add_command(label="Remove Texture", command=self.remove_nut_texture)
    ...
    def update_name_path(self, event):
        ...
        if len(selection) > 0:
            ...
            if parent == '':
                ...
                self.update_text_variables(texture.nut.textures[0].height, texture.nut.textures[0].width, texture.nut.textures[0].pixel_format, texture.nut.textures[0].mipmap_count)
            else:
                print(index)
                texture = textures[int(parent)]
                self.update_texture_preview(texture.nut.textures[int(index)])
                self.update_text_variables(texture.nut.textures[index].height, texture.nut.textures[index].width, texture.nut.textures[index].pixel_format, texture.nut.textures[index].mipmap_count)
    ...
    def export_nut(self):
        ...
        if len(selection) > 0:
            path = fd.askdirectory(title= 'Select a location to export the NUT texture',)
            if path != '':
    ...
    def export_dds(self):
        ...
        selection = [i for i in self.textures_list.selection() if self.textures_list.parent(i) == '']
        if len(selection) > 0:
            path = fd.askdirectory(title= 'Select a location to export the DDS texture')
            if path != '':
                for tex in selection:
                    texture: NuccChunkTexture = textures[self.textures_list.index(tex)]
    ...
    def export_png(self):
        ...
        selection = [i for i in self.textures_list.selection() if self.textures_list.parent(i) == '']
        if len(selection) > 0:
            path = fd.askdirectory(title= 'Select a location to export the PNG texture',)
            if path != '':
                for tex in selection:
                    texture: NuccChunkTexture = textures[self.textures_list.index(tex)]
    ...
    def add_texture(self):
        ...
        for file in files:
            if file != '':
                ...
                self.textures_list.insert(index, 'end', text= f'{len(texture.nut.textures)-1}')
    ...
    def replace_texture(self):
        ...
        if file != '':
            ...
            texture = textures[index]
            texture.nut.textures[self.textures_list.index(selected)] = DDS_to_NutTexture(dds)
            self.update_texture_preview(texture.nut.textures[self.textures_list.index(selected)])
            self.update_name_path(None)
            self.update_text_variables(dds.header.height, dds.header.width, 0, dds.header.mipMapCount)
        print('done')
